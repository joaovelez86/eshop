...
<?php if(is_object($profile_data)): ?>
	...
	<div class="col-md-4 mb" style="flex:1;background-color: #eee;text-align: center;box-shadow: 0px 0px 20px #aaa; border: solid thin #ddd;">
		...
		<div class="white-panel pn">
			...
			<p><b><?=$profile_data->full_name?></b></p>
			<div class="row">
				<div class="col-md-6">
			...
			<table class="table">
				...
				<tbody onclick="show_details(event)">
					<?php foreach($orders as $order):?>
						...
						?>
						<tr style="position: relative;"><td><?=$order->id?></td><td><?=date("d/m/y H:i",strtotime($order->date))?></td><td><?=$order->total?>€</td><td><?=$order->delivery_address?></td><td><?=$order->state?></td><td><?=$order->mobile_phone?><td><?=$status?></td>
							<td>
