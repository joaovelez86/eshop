...
Class Slider
{
	...
	public function create($DATA,$FILES,$image_class = null)
	{
		...
		if(empty($arr['header1_text']) || !preg_match("/^[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,.'-]+$/u", trim($arr['header1_text'])))
		{
			$this->error .= "Por favor, inserir header1 válido <br>";
		}

		if(empty($arr['header2_text']))
		{
			$this->error .= "Por favor, inserir header2 válido <br>";
		}

		if(empty($arr['text']))
		{
			$this->error .= "Por favor, inserir Mensagem válida <br>";
		}

		if(empty($arr['link']))
		{
			$this->error .= "Por favor, inserir link válido <br>";
		}
		...
		if($this->error == ""){
			...
			foreach ($FILES as $key => $img_row) {
				...
				if($img_row['error'] == 0 && in_array($img_row['type'], $allowed))
				{
					...
					{
						$this->error .= $key . " é maior que o ramanho requerido <br>";
					}
				}
